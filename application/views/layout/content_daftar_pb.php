...
<div id="edit_pb" class="row" style="display:none;">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            table = $('#table').DataTable({ 
                ...
                "ajax": {
                    "url": "<?php echo site_url('inventory/ajax_list_pb')?>",
                    "type": "POST"
                },
        ...
        function delete_pb(id)
        {
            var x = document.getElementById("edit_pb");
            ...
            swal({
                    ...
                    $.ajax({
                    url : "<?php echo site_url('inventory/ajax_delete_pb')?>/"+id,
                    type: "POST",
                    dataType: "JSON",
        ...
        function edit_pb(id)
        {
            ...
            $.ajax({
                url : "<?php echo site_url('inventory/ajax_edit_pb')?>/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data)
                {                                              
                    $('[name="noPB"]').val(data.pb.no_PB);
                    $('[name="gudang"]').val(data.pb.id_gudang).trigger('change');
                    $('[name="departemen"]').val(data.pb.id_departemen).trigger('change');
                    $('[name="peminta"]').val(data.pb.id_pegawai).trigger('change');
                    $('[name="keterangan"]').val(data.pb.keterangan).trigger('change');
                    $('[name="kategori"]').val(data.pb.id_kategori).trigger('change');
                    getBarang();
                    
                    for (var i = 1, l = data.pbDetail.length; i <= l; i++) {
                        var kode = data.pbDetail[i-1].kode_barang;
                        var barang = data.pbDetail[i-1].nama_barang;
                        var deskripsi = data.pbDetail[i-1].keterangan;
                        var jumlah = data.pbDetail[i-1].jumlah;
                        var satuan =  data.pbDetail[i-1].nama_satuan;
                        var markup = "<tr><td class='text-center'><input type='checkbox' name='record'></td><td class='text-center'>" + kode + "</td><td>" + barang + "</td><td>"+ deskripsi +"</td><td class='text-center'>" + jumlah + "</td><td class='text-center'>"+ satuan + "</td></tr>";
                        $("#tableBarang tbody").append(markup);
                    }
        ...
        {
            $.ajax({
            url : "<?php echo site_url('inventory/ajax_update_pb')?>",
            type: "POST",
            data: $('#form').serialize(),
            dataType: "JSON",
            success: function(data)
            {
                if (data.inputerror)
                {
                    ...
                    table.find('tr').each(function (i, el){
                        ...
                        $.ajax({
                            url : "<?php echo site_url('inventory/ajax_update_pb_detail')?>",
                            type: "POST",
                            dataType: "JSON",
                            data: {
                                no_PB: noPB,
