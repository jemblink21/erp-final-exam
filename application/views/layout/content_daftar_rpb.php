<div class="right_col" role="main">
  ...
  <div id="edit_rpb" class="row" style="display:none;">
    <div class="col-md-12 col-sm-12 col-xs-12"><!--awal div sub kotak -->
      <div class="x_panel"><!-- awal div x_panel -->
        <div class="x_title"><!-- awal div x_title -->
          <h2>Edit <i>Return</i> Penerimaan Barang</h2>
        ...
        <div class="x_content"><!-- awal div x_content -->
            <form action="#" id="form" class="form-horizontal form-label-left" novalidate><!-- awal form -->
                <div class="item form-group">
                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="noRPB">
                        No. Return Penerimaan Barang <span class="required">*</span>
                    </label>
                    <div class="col-md-2 col-sm-3 col-xs-6">
                        <input type="text" id="noRPB" name="noRPB" 
  ...
  <div class="row">
	...
	<div class="x_panel">
	  <div class="x_title">
	    <h2>Daftar </i>Return</i> Pembelian Barang</h2>
	    <ul class="nav navbar-right panel_toolbox">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            table = $('#table').DataTable({ 
                ...
                "ajax": {
                    "url": "<?php echo site_url('purchase/ajax_list_rpb')?>",
                    "type": "POST"
                },
        ...
        function edit_rpb(id){
            resetForm();
            clearTable();

            $.ajax({
                url : "<?php echo site_url('purchase/ajax_edit_rpb')?>/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data)
                {                                              
                    $('[name="noRPB"]').val(data.rpb.no_rpb);
                    $('[name="gudang"]').val(data.rpb.id_gudang).trigger('change');
                    $('[name="supplier"]').val(data.rpb.id_supplier).trigger('change');
                    getBPB();
                    $('.select2_bpb')
                        // .empty() //empty select
                        .append($("<option/>") //add option tag in select
                            .val(data.rpb.no_bpb) //set value for option to post it
                            .text(data.rpb.no_bpb)) //set a text for show in select
                        .val(data.rpb.no_bpb) //select option of select2
                        .trigger("change");
                    $('[name="noPO"]').val(data.rpb.no_po).trigger('change');
                    $('[name="keterangan"]').val(data.rpb.keterangan).trigger('change');
                    
                    clearBarang();
                    for (var i = 1, l = data.rpbDetail.length; i <= l; i++) {
                        var no_bpb_detail = data.rpbDetail[i-1].no_bpb_detail;
                        var no_po = data.rpbDetail[i-1].no_po;
                        var kode = data.rpbDetail[i-1].kode_barang;
                        var namaBarang = data.rpbDetail[i-1].nama_barang;
                        var deskripsi = data.rpbDetail[i-1].deskripsi;
                        var jumlah = data.rpbDetail[i-1].jumlah;
                        var satuan = data.rpbDetail[i-1].nama_satuan;
                        var markup = "<tr><td class='id_bpb'>"+ no_bpb_detail +"</td><td class='text-center'><input type='checkbox' name='record'></td><td class='text-center'>" + no_po + "</td><td class='text-center'>" + kode + "</td><td>"+ namaBarang +"</td><td>" + deskripsi + "</td><td class='text-center'>"+ jumlah + "</td><td class='text-center'>"+ satuan + "</td></tr>";
                        $("#tableBarang tbody").append(markup);
                        $('.id_bpb').hide();
                    }
                    
                    $('html,body').scrollTop(0);
                    var x = document.getElementById("edit_rpb");
        ...
        function save(){
            $.ajax({
            url : "<?php echo site_url('purchase/ajax_update_rpb')?>",
            type: "POST",
            data: $('#form').serialize(),
            dataType: "JSON",
            success: function(data)
            {
                if (data.inputerror)
                {
                    ...
                    $('html, body').animate({ scrollTop: $('#edit_rpb').offset().top }, 'slow');
        ...
        function delete_rpb(id)
        {
            ...
            swal({
                ...
                closeOnConfirm: false
                    ...
                    $.ajax({
                    url : "<?php echo site_url('purchase/ajax_delete_rpb')?>/"+id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data)
                        {
